...
def compare_cor_across_dcs(
    ...
    for cor_switch, has_vlans in xdc_cor_switch_vlans.items():
        ...
        results.append({
            ...
            "Switch_Type": "COR_ALL_DC",     # distinct label for cross-DC results
            "Switch": cor_switch,
            "Has_VLANs": sorted(has_vlans, key=int),
            "Missing_VLANs": sorted(missing_vlans, key=int),
        })
...
# def get_firewall_vlan():
#     loop through "tag" in the firewall interfaces
#     return firewall vlan
#
# def compare_cor_vlan_to_firewalls():
#     compare firewall vlan with cor switch vlan
...
def compare_zone_vlans_tor_vs_cor(vlan_data, zone_vrf_data, zone_name):
    ...
    - Finally (new): compare COR switches across BOTH DCs against the global COR union baseline
    """
    results = []
    ...
    for dc in ["DC1", "DC2"]:
        ...
        # NEW: Accumulate for cross-DC comparison
...
def main(out_dir: Path = OUTPUT_DIR,
    ...
    if save_consolidated and zone_dfs:
        consolidated_path = out_dir / "zone_vlan_comparison_all.xlsx"
